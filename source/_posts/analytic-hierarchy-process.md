...
同样的，为了方便说明，我们作如下假设：
- $w_{供应商-产品价格}=(0.4, 0.3, 0.3)^T$
- $w_{供应商-运输费用}=(0.2, 0.4, 0.4)^T$
- $w_{供应商-项目管理能力}=(0.2, 0.5, 0.3)^T$
- $w_{供应商-在大众中的口碑}=(0.4, 0.3, 0.3)^T$
- $w_{供应商-在甲方中的口碑}=(0.3, 0.5, 0.2)^T$
- $w_{供应商-工作人员的水平和能力}=(0.2, 0.3, 0.5)^T$
- $w_{供应商-售后处理速度}=(0.3, 0.3, 0.4)^T$
- $w_{供应商-售后处理方案}=(0.3, 0.3, 0.4)^T$
- $w_{供应商-现货数量}=(0.2, 0.3, 0.5)^T$
- $w_{供应商-交付及时性}=(0.2, 0.3, 0.5)^T$
- $w_{供应商-供货稳定性}=(0.3, 0.2, 0.5)^T$
- $w_{供应商-对接多少采购方}=(0.5, 0.2, 0.3)^T$
- $w_{供应商-在采购方中的优先级}=(0.3, 0.4, 0.3)^T$
- $w_{供应商-其他采购方采购数量}=(0.4, 0.3, 0.3)^T$
...
$$
其中$a_i$表示我们求得的表中14个总权重，$CI_i$表示我们在求解14个$w_{供应商-xxxx}$权重向量的过程中进行一致性检验得到的14个$CI$，同理$RI_i$也是我们在做那些一致性检验过程中查表获得的每个$RI$。
